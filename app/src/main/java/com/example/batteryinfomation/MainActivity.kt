        ...
        val activityLauncher =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    if (SystemSettingUtil.getInstance().canWrite(this)) {
                        when (requestFrom) {
...
//                            viewModel.setBrightness(screenBrightnessSlider.value / screenBrightnessSlider.valueTo)
                ...
                when (requestFrom) {
                    REQUEST_FROM_USAGE_ACCESS -> {
                        if (PermissionChecker.getInstance().hasUsageAccessPermission(this)) {
        ...
        hapticFeedbackSwitch.setOnClickListener {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !SystemSettingUtil.getInstance().canWrite(
                    this
                )) {
                hapticFeedbackSwitch.isChecked = !hapticFeedbackSwitch.isChecked
                requestFrom = REQUEST_FROM_HAPTIC_FEEDBACK
                activityLauncher.launch(
                    SystemSettingUtil.getInstance().getManageWriteSettingsIntent(
                        packageName
                    )
                )
            } else {
        ...
        screenBrightnessSlider.addOnSliderTouchListener(object : Slider.OnSliderTouchListener {
            override fun onStartTrackingTouch(slider: Slider) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !SystemSettingUtil.getInstance()
                        .canWrite(
                            this@MainActivity
                        )
                ) {
        ...
        appUsageBtn.setOnClickListener {
            if (!PermissionChecker.getInstance().hasUsageAccessPermission(this)) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    ...
                    activityLauncher.launch(PermissionChecker.getInstance().getUsageAccessSettingIntent())
                }
            } else {
                startActivity(Intent(this, AppUsageActivity::class.java))
            }
//            val batterySaver = Intent(Intent.ACTION_POWER_USAGE_SUMMARY)
//
...
//        Log.d("xxxxx", "dockState:$dockState")
        ...
        batteryTotalCapacity = BatteryUtil.getInstance().getBatteryTotalCapacity(this)
        batteryTotalCapacityTv.text = String.format(
            getString(R.string.total_capacity),
            batteryTotalCapacity.toInt().toString() + "mAh"
        )

        viewModel = BatteryApp.getInstance().getBatteryViewModel();
        viewModel
            .getBatteryInfo()
            .observe(this, {
                updateUI(it as BatteryInfo)
            })
        ...
        viewModel.isHapticFeedbackEnabled().observe(this, {
            hapticFeedbackSwitch.isChecked = it as Boolean
        })

        viewModel.isSoundEffectsEnabled().observe(this, {
            soundEffectsSwitch.isChecked = it as Boolean
        })

        viewModel.brightnessValue.observe(this, {
            screenBrightnessSlider.value =
                it as Int / 255f * (screenBrightnessSlider.valueTo - screenBrightnessSlider.valueFrom)
        })

        viewModel.screenOffTimeoutValue.observe(this, {
            val value = it as Int / 1000
